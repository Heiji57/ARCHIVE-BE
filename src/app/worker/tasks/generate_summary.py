...
from app.retrospective.infrastructure.ai.prompt_builder import build_prompt
...
async def generate_summary_task(summary_id: str, user_id: str, send_notification: bool = True) -> None:
    settings = get_settings()
    factory = get_worker_session_factory()
    redis = Redis.from_url(settings.redis.cache_url, decode_responses=True)

    try:
        # ── Transaction 1: mark in_progress + fetch entries ──────────────────
        ...
        prompt = build_prompt(summary.summary_type, entries)
